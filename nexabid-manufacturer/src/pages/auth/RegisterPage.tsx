...
import {
  ...
  Building2, FileText, ChevronDown, Factory, Mail, Shield,
} from 'lucide-react'
    ...
    <div className="flex gap-2 mt-2">
      {checks.map((check) => (
        <div key={check.label} className="flex items-center gap-1">
          <div className={cn('w-1.5 h-1.5 rounded-full transition-colors', check.valid ? 'bg-green-500' : 'bg-gray-200')} />
          <span className={cn('text-xs transition-colors', check.valid ? 'text-green-600' : 'text-gray-400')}>
            {check.label}
          </span>
        </div>
      ))}
...
export default function RegisterPage() {
  ...
  const handleResend = async () => {
    ...
    try { await api.post('/auth/send-verification', { email: pendingEmail }); startResendCooldown() }
    catch { /* silent */ }
  }
      ...
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm space-y-5">
            ...
            <button onClick={handleVerify} disabled={verifyLoading || otp.length !== 6}
              className="w-full py-3.5 bg-[#0A0A0A] text-white font-semibold rounded-xl hover:bg-[#1a1a1a] transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            <div className="text-center">
              <p className="text-sm text-gray-500">
                ...
                <button onClick={handleResend} disabled={resendCooldown > 0}
                  className="font-semibold text-[#0A0A0A] hover:underline disabled:text-gray-400">
                  {resendCooldown > 0 ? `Resend in ${resendCooldown}s` : 'Resend'}
                </button>
              </p>
            </div>
            <div className="pt-2 border-t border-gray-100 text-center">
              <button onClick={() => { setVerifyStep(false); setOtp(''); setVerifyError('') }}
        ...
        <div className="relative z-10 space-y-6">
          <div>
            <p className="text-white/40 text-xs uppercase tracking-[0.2em] font-medium mb-4">
              Join 1,200+ manufacturers
            </p>
            <h1 className="text-white text-3xl font-semibold leading-[1.15] tracking-[-0.03em]">
              Your factory deserves
              <br />
              <span className="text-white/45">better orders.</span>
            </h1>
          </div>

          {/* Earnings preview card */}
          <div className="bg-white/[0.05] border border-white/[0.09] rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-white/50 text-xs uppercase tracking-widest">Live on platform</span>
            ...
            <div className="space-y-3">
              {[
                { category: 'Textiles', orders: 48, value: '₹12L–₹45L' },
                { category: 'Electronics', orders: 31, value: '₹8L–₹30L' },
                { category: 'Hardware', orders: 27, value: '₹5L–₹22L' },
              ].map((item) => (
                <div key={item.category} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-white/30" />
                    <span className="text-white/70 text-sm">{item.category}</span>
                    <span className="text-white/30 text-xs">{item.orders} orders</span>
                  </div>
          ...
          <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl p-5">
            <p className="text-white/70 text-sm leading-relaxed">
              "I got my first order within 2 days of joining. The escrow system made me feel safe to start immediately."
            </p>
            <div className="flex items-center gap-3 mt-4">
              ...
              <div>
                <div className="text-white text-xs font-medium">Amit Sharma</div>
                <div className="text-white/40 text-xs">Textile Manufacturer, Surat</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-10 lg:px-14 xl:px-20 py-10 overflow-y-auto">
        ...
        <div className="max-w-[440px] w-full mx-auto lg:mx-0">
          ...
          <div className="mb-7 animate-fade-up">
            ...
            <p className="text-gray-500 text-sm">
              Start accepting manufacturing orders today
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 animate-fade-up animate-delay-300">
            ...
            <button
              ...
              className="flex items-center gap-2 text-sm text-gray-500 hover:text-black transition-colors pt-1 w-full"
            >
              <Factory size={15} />
            ...
            {/* Optional Fields */}
            {showOptional && (
              <div className="space-y-4 pt-1 border-t border-gray-100 animate-fade-up">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      {...register('category')}
                      defaultValue=""
                      className="w-full px-4 py-3 pr-10 rounded-xl border border-gray-200 focus:border-gray-900 focus:ring-2 focus:ring-gray-100 text-sm transition-all duration-200 outline-none appearance-none bg-white text-gray-700"
                    >
