...
const pageTitles: Record<string, string> = {
  '/dashboard': 'Dashboard', '/browse': 'Browse Orders',
  '/orders': 'My Orders', '/chat': 'Messages',
...
export default function Topbar({ onMenuClick }: { onMenuClick?: () => void }) {
  const { manufacturer, token } = useAuthStore()
  ...
  useEffect(() => { if (!manufacturer) disconnectSocket() }, [manufacturer])
    ...
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-6 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="w-8 h-8 rounded-full bg-[#0A0A0A] flex items-center justify-center ml-1 cursor-pointer">
          <span className="text-white text-xs font-semibold">{manufacturer?.fullName?.charAt(0).toUpperCase()}</span>
        </div>
      </div>
    </header>
  )
