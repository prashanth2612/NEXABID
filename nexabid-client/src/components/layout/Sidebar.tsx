...
const navItems = [
  {
    ...
    items: [
      ...
      { label: 'Post Order',   icon: PlusCircle,      path: '/orders/create' },
    ],
  },
  {
    group: 'Activity',
...
export default function Sidebar({ open, onClose }: { open?: boolean; onClose?: () => void }) {
  ...
  return (
    <aside className={`w-[240px] min-h-screen bg-[#0A0A0A] flex flex-col fixed left-0 top-0 bottom-0 z-40 transition-transform duration-300 ease-in-out
    ...
    `}>
      ...
      <div className="h-16 flex items-center px-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="block text-white/30 text-[10px] tracking-wide uppercase">Client Portal</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 pb-4 border-t border-white/[0.06] pt-3">
        ...
        <button
          ...
          className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-sm text-white/40 hover:text-red-400 hover:bg-red-400/10 transition-all duration-150"
        >
          <LogOut size={14} />
          <span>Sign out</span>
        </button>
